...
# Public repository placeholders; override with positional arguments.
DEFAULT_USER="OWNER"
...
def load_ui_cfg():
    ...
    cfg = {"host": "::", "port": 8787, "secret_path": "<SECRET_PATH>", "password": ""}
...
def print_status():
    ...
    secret_path = cfg.get("secret_path", "<SECRET_PATH>")
...
def get_status_state():
    ...
    return (
        cfg.get("port", 8787),
        cfg.get("secret_path", "<SECRET_PATH>"),
...
AUTH_FILE="${INSTALL_DIR}/vpngate_data/ui_auth.json"
if [ -f "$AUTH_FILE" ]; then
    SECRET_PATH=$(python3 -c "import json; print(json.load(open('$AUTH_FILE')).get('secret_path', '<SECRET_PATH>'))" 2>/dev/null || echo "<SECRET_PATH>")
