...
STRICT_RESIDENTIAL_ONLY = env_bool("STRICT_RESIDENTIAL_ONLY", True)
STANDBY_TARGET = env_int("STANDBY_TARGET", 10, 1, 20)
STANDBY_MAX_AGE_SECONDS = env_int("STANDBY_MAX_AGE_SECONDS", 8 * 24 * 3600, 30)
STANDBY_SCAN_BATCH_SIZE = env_int("STANDBY_SCAN_BATCH_SIZE", 10, 1, 50)
STANDBY_SCAN_LIMIT = env_int("STANDBY_SCAN_LIMIT", 20, 1, 300)
TEST_MAX_WORKERS = env_int("TEST_MAX_WORKERS", 2, 1, 2)
POOL_SOURCE_MODE = os.environ.get("POOL_SOURCE_MODE", "local-authority").strip().lower()
UPSTREAM_SNAPSHOT_MAX_AGE_SECONDS = env_int("UPSTREAM_SNAPSHOT_MAX_AGE_SECONDS", 7200, 60)
UPSTREAM_SNAPSHOT_MAX_BYTES = env_int("UPSTREAM_SNAPSHOT_MAX_BYTES", 8 * 1024 * 1024, 1024)
UPSTREAM_SNAPSHOT_MAX_NODES = env_int("UPSTREAM_SNAPSHOT_MAX_NODES", 300, 1)
UPSTREAM_PROBE_COOLDOWN_SECONDS = env_int("UPSTREAM_PROBE_COOLDOWN_SECONDS", 1800, 1)
UPSTREAM_LOCAL_PROBE_BATCH_SIZE = env_int("UPSTREAM_LOCAL_PROBE_BATCH_SIZE", 10, 1, 10)
UPSTREAM_LOCAL_PROBE_INTERVAL_SECONDS = env_int("UPSTREAM_LOCAL_PROBE_INTERVAL_SECONDS", 300, 1)
UPSTREAM_LOCAL_PROBE_BYTES = env_int("UPSTREAM_LOCAL_PROBE_BYTES", 25_000_000, 1_000_000, 50_000_000)
UPSTREAM_LOCAL_PROBE_SAMPLES = env_int("UPSTREAM_LOCAL_PROBE_SAMPLES", 1, 1, 3)
UPSTREAM_LOCAL_PROBE_RETENTION_SECONDS = env_int("UPSTREAM_LOCAL_PROBE_RETENTION_SECONDS", 172800, 60)
DAILY_MAINTENANCE_START_HOUR = env_int("DAILY_MAINTENANCE_START_HOUR", 3, 0, 23)
DAILY_MAINTENANCE_END_HOUR = env_int("DAILY_MAINTENANCE_END_HOUR", 4, 1, 24)
EMERGENCY_REFILL_COOLDOWN_SECONDS = env_int("EMERGENCY_REFILL_COOLDOWN_SECONDS", 6 * 3600, 300)
STABLE_POOL_PROBE_DAILY_LIMIT = env_int("STABLE_POOL_PROBE_DAILY_LIMIT", 2, 0, 20)
STABLE_POOL_PROBE_BYTES = env_int("STABLE_POOL_PROBE_BYTES", 25_000_000, 1_000_000, 50_000_000)
STABLE_POOL_PROBE_UPLOAD_BYTES = env_int("STABLE_POOL_PROBE_UPLOAD_BYTES", 3_000_000, 0, 10_000_000)
STABLE_POOL_PROBE_SAMPLES = env_int("STABLE_POOL_PROBE_SAMPLES", 2, 1, 3)
FULL_PROBE_BYTES = env_int("FULL_PROBE_BYTES", 10_000_000, 1_000_000, 50_000_000)
QUALITY_TIE_WINDOW = env_int("QUALITY_TIE_WINDOW", 5, 0, 20)
SPEED_SWITCH_GAIN_PERCENT = env_int("SPEED_SWITCH_GAIN_PERCENT", 30, 5, 300)
SWITCH_REQUIRED_WINS = env_int("SWITCH_REQUIRED_WINS", 2, 1, 5)
FULL_PATH_SPEEDTEST_CMD = os.environ.get(
    "FULL_PATH_SPEEDTEST_CMD", "python3 /opt/aimilivpn/scripts/full_path_speedtest.py"
)
...
SWITCH_COOLDOWN_SECONDS = env_int("SWITCH_COOLDOWN_SECONDS", 600, 1)
NODE_FAILURE_COOLDOWN_SECONDS = env_int("NODE_FAILURE_COOLDOWN_SECONDS", 21600, 1)
VPNGATE_PROBE_FAIL_THRESHOLD = env_int("VPNGATE_PROBE_FAIL_THRESHOLD", 2, 1)
VPNGATE_PROBE_INTERVAL_SECONDS = env_int("VPNGATE_PROBE_INTERVAL_SECONDS", 5, 0)
FRONTEND_PROBE_FAIL_THRESHOLD = env_int("FRONTEND_PROBE_FAIL_THRESHOLD", 2, 1)
FRONTEND_PROBE_MIN_INTERVAL_SECONDS = env_int("FRONTEND_PROBE_MIN_INTERVAL_SECONDS", 60, 0)
FRONTEND_PROBE_CONFIG = os.environ.get("FRONTEND_PROBE_CONFIG") or None
STANDBY_SLOT_MAX_FAILURES = env_int("STANDBY_SLOT_MAX_FAILURES", 3, 1, 10)
STANDBY_SLOT_PAUSE_SECONDS = env_int("STANDBY_SLOT_PAUSE_SECONDS", 600, 60)
...
UPSTREAM_SNAPSHOT_FILE = Path(
    os.environ.get("UPSTREAM_SNAPSHOT_FILE", str(DATA_DIR / "upstream-snapshot.json"))
)
LOCAL_PROBE_STATE_FILE = DATA_DIR / "local-probe-state.json"

lock = threading.RLock()
maintenance_lock = threading.Lock()
speed_traffic_lock = threading.Lock()
active_sessions: dict[str, float] = {}
SLOT_RUNTIME: dict[str, dict[str, Any]] = {
    "A": {"process": None, "node_id": None, "draining_since": None},
    "B": {"process": None, "node_id": None, "draining_since": None},
}
# 旧全局名单隧道态的兼容镜像：始终指向当前 active 槽的运行时状态，
# 由 _sync_legacy_active_state() 在每次槽位运行时变更后同步。
active_openvpn_process: subprocess.Popen[str] | None = None
active_openvpn_node_id = ""
startup_preferred_node_id = ""
is_connecting = False
standby_replenish_lock = threading.Lock()
standby_slot_lock = threading.Lock()
# 故障归因重入保护：collector/checker 等并发触发时只有一个执行归因
attribution_lock = threading.Lock()
# standby 槽建备节制：同一槽连续 STANDBY_SLOT_MAX_FAILURES 次失败后暂停 STANDBY_SLOT_PAUSE_SECONDS
standby_slot_failures: dict[str, int] = {"A": 0, "B": 0}
standby_slot_paused_until: dict[str, float] = {"A": 0.0, "B": 0.0}
...
def _slot_process_dead(state: dict[str, Any]) -> bool:
    """槽位进程视为死亡：无进程对象，或 poll() 已返回退出码（含外部 kill 的残留对象）。"""
...
def current_active_slot() -> str | None:
    """返回当前 active 槽：优先活动槽指针；无指针时回退到仍有存活隧道的槽。"""
    try:
        pointer = slot_state.read_active_slot(str(DATA_DIR))
    except Exception:
        pointer = None
    if pointer and pointer.get("slot") in SLOT_RUNTIME:
        return str(pointer["slot"])
    for candidate in SLOT_RUNTIME:
        state = SLOT_RUNTIME[candidate]
        if not _slot_process_dead(state) or (state["process"] is None and state["node_id"]):
            return candidate
    return None

def _sync_legacy_active_state() -> None:
    """把 active_openvpn_process/active_openvpn_node_id 镜像到当前 active 槽。"""
    global active_openvpn_process, active_openvpn_node_id
    slot = current_active_slot()
    if slot is not None:
        active_openvpn_process = SLOT_RUNTIME[slot]["process"]
        active_openvpn_node_id = str(SLOT_RUNTIME[slot]["node_id"] or "")
    else:
...
def cleanup_test_artifacts() -> None:
    """Remove only AimiliVPN test artifacts; never touch production tun0/tun1."""
    try:
        TEMP_DIR.mkdir(exist_ok=True, parents=True)
        ...
        if sys.platform.startswith("linux"):
            for idx in range(2, 100):
                subprocess.run(["ip", "rule", "del", "priority", str(12000 + idx)], capture_output=True, timeout=1)
                subprocess.run(["ip", "route", "flush", "table", str(2000 + idx)], capture_output=True, timeout=1)
    except Exception as exc:
        print(f"[清理错误] 测试残留清理失败: {exc}", flush=True)
        ...
        if auth_file.exists():
            try:
                ...
                for key, val in data.items():
                    config[key] = val
                for key in ["host", "port", "proxy_port", "routing_mode", "force_country", "routing_ip_type", "connection_enabled", "fixed_node_id", "favorite_node_ids", "favorite_opt_out_ids", "fav_fail_fallback"]:
                    if key not in data:
                        updated = True
            except Exception:
                pass
            ...
            if match:
                ...
                try:
                    ...
                    if now - path.stat().st_mtime > three_days_sec:
                        with lock:
                            path.unlink()
        files = sorted((path for path in logs_dir.glob("*.json") if path.is_file()), key=lambda path: path.stat().st_mtime)
        total = sum(path.stat().st_size for path in files)
        while total > 100 * 1024 * 1024 and len(files) > 1:
            oldest = files.pop(0)
...
def record_speed_probe_traffic(down_bytes: int = 0, up_bytes: int = 0, path: str = "") -> None:
    """Record observed test traffic for visibility; this is accounting, not a hard budget."""
    now = time.time()
    month = time.strftime("%Y-%m", time.localtime(now))
    day = time.strftime("%Y-%m-%d", time.localtime(now))
    with speed_traffic_lock, lock:
        state = read_json(STATE_FILE, {})
        if state.get("speed_probe_month") != month:
            state["speed_probe_month"] = month
            state["speed_probe_month_bytes"] = 0
        if state.get("speed_probe_day") != day:
            state["speed_probe_day"] = day
            state["speed_probe_day_bytes"] = 0
        transferred = max(0, int(down_bytes or 0)) + max(0, int(up_bytes or 0))
        state["speed_probe_month_bytes"] = int(state.get("speed_probe_month_bytes", 0) or 0) + transferred
        state["speed_probe_day_bytes"] = int(state.get("speed_probe_day_bytes", 0) or 0) + transferred
        state["last_speed_probe_at"] = now
        state["last_speed_probe_path"] = path
        write_json(STATE_FILE, state)

def run_daily_full_path_speedtest() -> bool:
    """Test the existing direct and VPNGate XHTTP/CDN subscriptions once per day."""
    today = time.strftime("%Y-%m-%d", time.localtime())
    state = get_state()
    if state.get("last_full_path_speedtest_date") == today:
        return bool(state.get("last_full_path_speedtest_ok"))
    try:
        result = subprocess.run(
            shlex.split(FULL_PATH_SPEEDTEST_CMD), capture_output=True, text=True, timeout=240,
        )
        report = json.loads((result.stdout or "").strip().splitlines()[-1])
        paths = report.get("paths") if isinstance(report.get("paths"), dict) else {}
        down = sum(int(value.get("bytes_down", 0) or 0) for value in paths.values() if isinstance(value, dict))
        up = sum(int(value.get("bytes_up", 0) or 0) for value in paths.values() if isinstance(value, dict))
        record_speed_probe_traffic(down, up, "xhttp-cdn-direct+xhttp-cdn-vpngate")
        ok = result.returncode == 0 and len(paths) == 2 and all(value.get("ok") for value in paths.values())
        set_state(
            last_full_path_speedtest_date=today,
            last_full_path_speedtest_at=time.time(),
            last_full_path_speedtest_ok=ok,
            full_path_speedtest=paths,
        )
        return ok
    except Exception as exc:
        set_state(
            last_full_path_speedtest_date=today,
            last_full_path_speedtest_at=time.time(),
            last_full_path_speedtest_ok=False,
            last_full_path_speedtest_error=str(exc),
        )
        log_to_json("WARNING", "Speed", f"XHTTP/CDN 全路径测速失败: {exc}")
        return False
...
def is_upstream_consumer() -> bool:
    return POOL_SOURCE_MODE == "upstream-consumer"

def load_upstream_snapshot(now: float | None = None) -> dict[str, Any]:
    return pool_snapshot.load_snapshot_file(
        UPSTREAM_SNAPSHOT_FILE,
        max_bytes=UPSTREAM_SNAPSHOT_MAX_BYTES,
        now=time.time() if now is None else now,
        max_age_seconds=UPSTREAM_SNAPSHOT_MAX_AGE_SECONDS,
        max_nodes=UPSTREAM_SNAPSHOT_MAX_NODES,
    )

def refresh_upstream_pool_state(persist: bool = True) -> dict[str, list[str]]:
    snapshot = load_upstream_snapshot()
    previous = {str(node.get("id")): node for node in read_nodes() if node.get("id")}
    probe_state = upstream_pool.load_probe_state(LOCAL_PROBE_STATE_FILE)
    nodes = upstream_pool.snapshot_nodes(snapshot, CONFIG_DIR, probe_state)
    for node in nodes:
        node_id = str(node.get("id") or "")
        local = probe_state.get(node_id, {})
        if upstream_pool.probe_is_current(probe_state, node_id, str(node.get("config_hash") or "")):
            node["probe_status"] = "available"
            node["probe_message"] = "云途本地隧道复核已通过"
            node["probed_at"] = float(local.get("last_probe_at", 0) or 0)
        elif float(local.get("cooldown_until", 0) or 0) > time.time():
            node["probe_status"] = "unavailable"
            node["probe_message"] = f"云途本地复核失败，冷却至 {int(local['cooldown_until'])}"
        old = previous.get(node_id)
        if old and old.get("active") and old.get("config_hash") == node.get("config_hash"):
            node["active"] = True
        node_pool.persist_config(node)
    cleaned_probe_state = upstream_pool.prune_probe_state(
        probe_state, nodes, time.time(), UPSTREAM_LOCAL_PROBE_RETENTION_SECONDS
    )
    if cleaned_probe_state != probe_state:
        write_json(LOCAL_PROBE_STATE_FILE, cleaned_probe_state)
    managed_ids = node_pool.managed_config_ids(CONFIG_DIR)
    try:
        node_pool.prune_managed_configs(
            CONFIG_DIR,
            managed_ids,
            {str(node.get("id")) for node in nodes if node.get("id")},
        )
    except OSError as exc:
        log_to_json("WARNING", "Pool", f"清理上游节点配置失败: {exc}")
    if persist:
        write_json(NODES_FILE, nodes)
    pools = snapshot["pools"]
    local_ranked = upstream_pool.rank_local_nodes(
        nodes,
        now=time.time(),
        speed_gain_percent=SPEED_SWITCH_GAIN_PERCENT,
    )
    local_standby_ids = [
        str(node.get("id"))
        for node in local_ranked
        if node.get("id") and not node.get("active")
    ][:STANDBY_TARGET]
    set_state(
        pool_source_mode=POOL_SOURCE_MODE,
        upstream_snapshot_sequence=snapshot["sequence"],
        upstream_snapshot_generated_at=snapshot["generated_at"],
        upstream_snapshot_source=snapshot["source_instance"],
        trusted_pool_ids=pools["trusted_ids"],
        observation_pool_ids=pools["observation_ids"],
        candidate_pool_ids=pools["candidate_ids"],
        standby_node_ids=local_standby_ids,
        trusted_pool_ready=len(pools["trusted_ids"]),
        observation_pool_ready=len(pools["observation_ids"]),
        candidate_pool_ready=len(pools["candidate_ids"]),
        standby_ready=len(local_standby_ids),
        standby_shortage=max(0, STANDBY_TARGET - len(local_standby_ids)),
    )
    return {
        "trusted": list(pools["trusted_ids"]),
        "observation": list(pools["observation_ids"]),
        "candidate": list(pools["candidate_ids"]),
        "standby": local_standby_ids,
    }

def refresh_pool_state(nodes: list[dict[str, Any]] | None = None, persist: bool = True) -> dict[str, list[str]]:
    if is_upstream_consumer():
        return refresh_upstream_pool_state(persist=persist)
    current = read_nodes() if nodes is None else nodes
    pools = node_pool.reconcile_pools(current)
    ui_cfg = load_ui_config()
    favorites = [str(value) for value in ui_cfg.get("favorite_node_ids", []) if value]
    opted_out = {str(value) for value in ui_cfg.get("favorite_opt_out_ids", []) if value}
    for node_id in node_pool.auto_favorite_ids(current, opted_out):
        if node_id not in favorites:
            favorites.append(node_id)
    ui_cfg["favorite_node_ids"] = favorites
    write_json(DATA_DIR / "ui_auth.json", ui_cfg)
    favorite_set = set(favorites)
    for node in current:
        if node_pool.should_persist_config(node, favorite_set, active_openvpn_node_id):
            try:
                node_pool.persist_config(node)
            except OSError as exc:
                log_to_json("WARNING", "Pool", f"保存节点配置失败 {node.get('id')}: {exc}")
    if persist:
        write_json(NODES_FILE, sort_all_nodes(current))
    state = read_json(STATE_FILE, {})
    state.update(
        trusted_pool_ids=pools["trusted"],
        observation_pool_ids=pools["observation"],
        trusted_pool_ready=len(pools["trusted"]),
        observation_pool_ready=len(pools["observation"]),
        trusted_pool_limit=node_pool.TRUSTED_LIMIT,
        observation_pool_limit=node_pool.OBSERVATION_LIMIT,
    )
    write_json(STATE_FILE, state)
    return pools

def _slot_public_view() -> dict[str, Any]:
    """汇总双槽只读视图：slot_states.json 为主、SLOT_RUNTIME 补 node_id；绝不包含 process 等运行时对象。"""
    try:
        saved = slot_state.read_slot_states(str(DATA_DIR))
    except Exception:
        saved = {}
    if not isinstance(saved, dict):
        saved = {}
    view: dict[str, Any] = {}
    for slot, device in slot_state.SLOT_DEVICES.items():
        info = saved.get(slot)
        if not isinstance(info, dict):
            info = {}
        runtime = SLOT_RUNTIME.get(slot, {})
        view[slot] = {
            "node_id": info.get("node_id") or runtime.get("node_id"),
            "device": str(info.get("device") or device),
...
def get_state() -> dict[str, Any]:
    ...
    state.setdefault("strict_residential_only", STRICT_RESIDENTIAL_ONLY)
    state.setdefault("standby_target", STANDBY_TARGET)
    state.setdefault("standby_ready", 0)
    state.setdefault("standby_shortage", STANDBY_TARGET)
    state.setdefault("standby_node_ids", [])
    state.setdefault("fail_closed", False)
    state.setdefault("fail_closed_reason", "")
    state.setdefault("trusted_pool_ids", [])
    state.setdefault("observation_pool_ids", [])
    state.setdefault("trusted_pool_limit", node_pool.TRUSTED_LIMIT)
    state.setdefault("observation_pool_limit", node_pool.OBSERVATION_LIMIT)
    state.setdefault("manual_override", False)

    # 双槽状态：active_slot/slots/draining_slot 每次从指针与运行时实时合并；
    # 切换冷却与轮换标记由切换编排持久化，缺省补默认值。
    state["active_slot"] = current_active_slot()
    state["slots"] = _slot_public_view()
    state["draining_slot"] = next(
        (
            slot
            for slot, runtime in SLOT_RUNTIME.items()
            if runtime.get("draining_since") is not None and slot != state["active_slot"]
        ),
        None,
    )
    state.setdefault("switch_cooldown_until", 0)
    state.setdefault("rotation_paused", False)
    state.setdefault("last_switch_at", None)
    state.setdefault("last_failover_reason", None)

    # Pre-populate settings inputs in UI
    ui_cfg = load_ui_config()
    state["username"] = ui_cfg.get("username", "admin")
    state["port"] = ui_cfg.get("port", 8787)
    state["secret_path"] = ui_cfg.get("secret_path", "<SECRET_PATH>")
...
def clear_active_connection_state(message: str) -> None:
    ...
    if slot is not None:
        with lock:
            stop_process(SLOT_RUNTIME[slot]["process"])
            SLOT_RUNTIME[slot]["process"] = None
            SLOT_RUNTIME[slot]["node_id"] = None
            SLOT_RUNTIME[slot]["draining_since"] = None
    _sync_legacy_active_state()
    with lock:
        nodes = read_nodes()
        for item in nodes:
            item["active"] = False
...
def cleanup_policy_routing(interface: str = "tun0", table: int = 100) -> None:
    ...
    try:
        subprocess.run(["ip", "rule", "del", "table", table_str], capture_output=True, timeout=2)
        subprocess.run(["ip", "route", "flush", "table", table_str], capture_output=True, timeout=2)
        print(f"[policy_routing] Cleared policy routing table {table_str} for interface {interface}", flush=True)
    except Exception:
        pass

def stop_slot_openvpn(slot: str) -> None:
    """只停指定槽的 OpenVPN 进程并清该槽路由规则，不影响其他槽。"""
    if slot not in SLOT_RUNTIME:
        raise ValueError(f"invalid slot: {slot!r}")
    with lock:
        cleanup_policy_routing(slot_state.SLOT_DEVICES[slot], slot_state.SLOT_TABLES[slot])
        stop_process(SLOT_RUNTIME[slot]["process"])
        SLOT_RUNTIME[slot]["process"] = None
        SLOT_RUNTIME[slot]["node_id"] = None
        SLOT_RUNTIME[slot]["draining_since"] = None
...
def stop_active_openvpn() -> None:
    """停当前 active 槽（保持旧签名）。无活动指针时停 SLOT_RUNTIME 中仍有隧道的槽。"""
    pointer_slot: str | None = None
    try:
        pointer = slot_state.read_active_slot(str(DATA_DIR))
        if pointer:
            pointer_slot = str(pointer["slot"])
    except Exception:
        pointer_slot = None
    if pointer_slot in SLOT_RUNTIME:
        stop_slot_openvpn(pointer_slot)
        return
    occupied = [
        slot for slot, state in SLOT_RUNTIME.items()
        if state["process"] is not None or state["node_id"]
    ]
    if occupied:
        print(f"[stop_active_openvpn] 无活动槽指针，停止仍有隧道的槽: {occupied}", flush=True)
        for slot in occupied:
            stop_slot_openvpn(slot)
        return
    for slot in SLOT_RUNTIME:
        cleanup_policy_routing(slot_state.SLOT_DEVICES[slot], slot_state.SLOT_TABLES[slot])
...
SLOT_EGRESS_PROBE_BYTES = env_int("SLOT_EGRESS_PROBE_BYTES", 1_000_000, 1_000_000, 50_000_000)

def verify_slot_egress(slot: str) -> dict[str, Any]:
    """实测指定槽隧道的出口 IP。consumer 模式校验出口 ≠ 本机默认公网且 ≠ 另一槽出口；不做住宅重评分。"""
    if slot not in SLOT_RUNTIME:
        raise ValueError(f"invalid slot: {slot!r}")
    ...
    egress = vpn_utils.probe_tunnel_egress(
        device,
        download_bytes=SLOT_EGRESS_PROBE_BYTES,
        upload_bytes=0,
        samples=1,
    )
    if not egress.get("ok"):
        result["error"] = str(egress.get("error") or "隧道出口探测失败")
        return result
    exit_ip = str(egress.get("exit_ip") or "")
    result["exit_ip"] = exit_ip
    result["country"] = str(egress.get("country") or "")
    result["ttfb_ms"] = int(egress.get("ttfb_ms") or 0)
    if not exit_ip:
        result["error"] = "隧道出口探测未返回出口 IP"
        return result
    if is_upstream_consumer():
        baseline_ip = direct_public_ip()
        if not baseline_ip:
            result["error"] = "无法取得云途默认公网出口基线，拒绝判定槽位出口"
            return result
        if exit_ip == baseline_ip:
            result["error"] = f"槽位 {slot} 出口 {exit_ip} 与云途默认公网相同"
            return result
    other_states = slot_state.read_slot_states(str(DATA_DIR))
    for other_slot, other_state in other_states.items():
        if other_slot == slot or not isinstance(other_state, dict):
            continue
        other_exit = str(other_state.get("exit_ip") or "")
        if other_exit and other_exit == exit_ip:
            result["error"] = f"槽位 {slot} 出口 {exit_ip} 与槽位 {other_slot} 的出口相同"
            return result
    result["ok"] = True
    return result

def _slot_process_alive(slot: str, data_dir: str) -> bool:
    """注入给 health_probes.probe_vpngate_slot 的进程存活检查：直接读 SLOT_RUNTIME。"""
    process = SLOT_RUNTIME.get(slot, {}).get("process")
    if process is None:
        return False
    try:
...
def _probe_slot_for_promotion(slot: str) -> dict[str, Any]:
    return health_probes.probe_vpngate_slot(
        slot,
        str(DATA_DIR),
        baseline_ip=direct_public_ip(),
        process_alive=_slot_process_alive,
    )

def promote_slot_to_active(slot: str, node_id: str) -> None:
    """唯一写活动指针的入口；写前必须 probe_vpngate_slot 通过，失败抛 RuntimeError 且不动旧指针。"""
    if slot not in SLOT_RUNTIME:
        raise ValueError(f"invalid slot: {slot!r}")
    probe = _probe_slot_for_promotion(slot)
    if not probe.get("ok"):
        raise RuntimeError(
            f"槽位 {slot} 提升前探针失败，保持旧活动指针: {probe.get('detail')}"
        )
    slot_state.write_active_slot(str(DATA_DIR), slot, node_id)
    _sync_legacy_active_state()

def _adopt_slot_after_connect(slot: str, node_id: str) -> None:
    """connect_node 验证通过后的活动指针接管。

    无有效指针、同槽换节点、或指针指向的槽已不健康时，经 promote_slot_to_active 接管；
    指针指向另一健康槽时保持 standby 不抢占。接管失败只记日志不推翻已建隧道。
    """
    try:
        pointer = slot_state.read_active_slot(str(DATA_DIR))
    except Exception:
        pointer = None
    pointer_slot = str(pointer.get("slot") or "") if isinstance(pointer, dict) else ""
    if pointer_slot == slot and str(pointer.get("node_id") or "") == node_id:
        return
    if pointer_slot and pointer_slot != slot and pointer_slot in SLOT_RUNTIME:
        try:
            probe = _probe_slot_for_promotion(pointer_slot)
        except Exception as exc:
            probe = {"ok": False, "detail": str(exc)}
        if probe.get("ok"):
            log_to_json("INFO", "Slot", f"活动指针指向健康槽 {pointer_slot}，槽位 {slot} 保持 standby")
            return
        log_to_json("WARNING", "Slot", f"活动指针槽 {pointer_slot} 已不健康，槽位 {slot} 接管: {probe.get('detail')}")
    try:
        promote_slot_to_active(slot, node_id)
        log_to_json("INFO", "Slot", f"槽位 {slot} 已接管活动指针（节点 {node_id}）")
    except Exception as exc:
        log_to_json("ERROR", "Slot", f"槽位 {slot} 接管活动指针失败，保持原指针: {exc}")

def _record_node_failure(node_id: str) -> None:
    """记节点失败 + NODE_FAILURE_COOLDOWN_SECONDS(6h) 冷却。故障矩阵 count_node_failure 路径与 standby 建备失败共用。"""
    ...
    if is_upstream_consumer():
        ...
        probe_state = upstream_pool.record_probe_result(
            upstream_pool.load_probe_state(LOCAL_PROBE_STATE_FILE),
            node_id,
            str(node.get("config_hash") or ""),
            False,
            "failover",
            time.time(),
            NODE_FAILURE_COOLDOWN_SECONDS,
        )
        write_json(LOCAL_PROBE_STATE_FILE, probe_state)
        return
    nodes = read_nodes()
    ...
    if node is None:
        return
    node["probe_status"] = "unavailable"
    node_pool.record_probe(node, False)
    write_json(NODES_FILE, nodes)

def _complete_failover(old_slot: str | None, old_node_id: str, new_slot: str, reason: str) -> None:
    """切换成功收尾：旧槽标 draining、记失败节点冷却、写切换冷却、触发补备。"""
    now = time.time()
    with lock:
        if old_slot and old_slot != new_slot and old_slot in SLOT_RUNTIME:
            SLOT_RUNTIME[old_slot]["draining_since"] = now
    if old_node_id:
        _record_node_failure(old_node_id)
    set_state(
        switch_cooldown_until=now + SWITCH_COOLDOWN_SECONDS,
        last_switch_at=now,
        last_failover_reason=reason,
        rotation_paused=False,
        performance_switch_paused=False,
    )
    _sync_legacy_active_state()
    schedule_standby_replenish()

def failover_to_standby() -> bool:
    """待机槽快速复验 → promote → 原 active 槽排空；无可用候选时清指针进入 fail-closed。"""
    active_slot = current_active_slot()
    old_node_id = str(SLOT_RUNTIME[active_slot]["node_id"] or "") if active_slot else ""
    target_slots = [slot for slot in SLOT_RUNTIME if slot != active_slot]
    if not target_slots:
        target_slots = list(SLOT_RUNTIME)

    for slot in target_slots:
        runtime = SLOT_RUNTIME[slot]
        if _slot_process_dead(runtime) or not runtime["node_id"]:
            continue
        try:
            promote_slot_to_active(slot, str(runtime["node_id"]))
        except RuntimeError as exc:
            log_to_json("WARNING", "Failover", f"待机槽 {slot} 复验失败，保持旧活动指针: {exc}")
            continue
        _complete_failover(active_slot, old_node_id, slot, "standby-recheck")
        return True

    connect_slot = target_slots[0]
    try:
        candidates = standby_nodes(read_nodes(), load_ui_config())
    except Exception as exc:
        log_to_json("WARNING", "Failover", f"读取切换候选失败: {exc}")
        candidates = []
    for node in candidates:
        node_id = str(node.get("id") or "")
        if not node_id or node_id == old_node_id:
            continue
        try:
            connect_node(node_id, slot=connect_slot)
            pointer_now = slot_state.read_active_slot(str(DATA_DIR))
            if not pointer_now or pointer_now.get("slot") != connect_slot:
                promote_slot_to_active(connect_slot, node_id)
        except Exception as exc:
            log_to_json("WARNING", "Failover", f"候选节点 {node_id} 切换失败，尝试下一候选: {exc}")
            continue
        _complete_failover(active_slot, old_node_id, connect_slot, f"pool-candidate:{node_id}")
        return True

    slot_state.clear_active_slot(str(DATA_DIR))
    _sync_legacy_active_state()
    enter_fail_closed("故障切换无可用候选，已进入 fail-closed")
    return False

def reap_drained_slots(now: float | None = None) -> list[str]:
    """排空清理：draining_since 超过 DRAIN_SECONDS 的非活动槽停止隧道，之后允许补备复用。"""
    checked_at = time.time() if now is None else now
    active_slot = current_active_slot()
    reaped: list[str] = []
    for slot, runtime in SLOT_RUNTIME.items():
        draining_since = runtime["draining_since"]
        if draining_since is None:
            continue
        if slot == active_slot:
            continue
        if checked_at - float(draining_since) < DRAIN_SECONDS:
            continue
        try:
            stop_slot_openvpn(slot)
        except Exception as exc:
            log_to_json("WARNING", "Drain", f"排空槽位 {slot} 失败: {exc}")
            continue
        reaped.append(slot)
    return reaped

def _load_frontend_probe_config() -> dict[str, Any]:
    if not FRONTEND_PROBE_CONFIG:
        return {}
    try:
        with open(FRONTEND_PROBE_CONFIG, encoding="utf-8") as fh:
            data = json.load(fh)
    except (OSError, ValueError):
        return {}
    return data if isinstance(data, dict) else {}

def _frontend_probe_consecutive(name: str, cfg: dict[str, Any] | None) -> bool:
    """前端探针连续 FRONTEND_PROBE_FAIL_THRESHOLD 次失败才判失败；未配置视为 ok。"""
    if not cfg:
        return True
    for _ in range(max(1, FRONTEND_PROBE_FAIL_THRESHOLD)):
        result = health_probes.probe_frontend(name, cfg)
        if result.get("ok"):
            return True
    return False

FRONTEND_WATCHDOG_STATE: dict[str, dict[str, Any]] = {}

def run_frontend_watchdog(now: float | None = None) -> dict[str, Any]:
    """健康分支低频前端观测：按 FRONTEND_PROBE_MIN_INTERVAL_SECONDS 节流，每个已配置前端探测一次。

    连续 FRONTEND_PROBE_FAIL_THRESHOLD 次失败写 last_alert 与 frontend_health
    （纯观测告警：绝不触碰活动指针、节点失败历史、rotation_paused）；
    恢复连续阈值次成功后清除该前端告警。未配置探针时零副作用。
    """
    checked_at = time.time() if now is None else now
    frontend_cfg = _load_frontend_probe_config()
    names = sorted(name for name, cfg in frontend_cfg.items() if cfg)
    result: dict[str, Any] = {"checked": [], "alerts": [], "checked_at": checked_at}
    if not names:
        return result
    probed = False
    for name in names:
        entry = FRONTEND_WATCHDOG_STATE.setdefault(
            name,
            {
                "fail_count": 0,
                "ok_count": 0,
        ...
        if checked_at - float(entry["last_probe_at"]) < FRONTEND_PROBE_MIN_INTERVAL_SECONDS:
            continue
        entry["last_probe_at"] = checked_at
        probed = True
        ok = bool(health_probes.probe_frontend(name, frontend_cfg[name]).get("ok"))
        entry["last_ok"] = ok
        result["checked"].append(name)
        if ok:
            entry["fail_count"] = 0
            entry["ok_count"] += 1
            if entry["alerted"] and entry["ok_count"] >= max(2, FRONTEND_PROBE_FAIL_THRESHOLD):
                entry["alerted"] = False
                current = get_state().get("last_alert") or {}
                if current.get("message") == entry["alert_message"]:
                    set_state(last_alert=None)
                log_to_json("INFO", "Frontend", f"{name} 前端入口已恢复，清除观测告警")
        else:
            entry["ok_count"] = 0
            entry["fail_count"] += 1
            if not entry["alerted"] and entry["fail_count"] >= FRONTEND_PROBE_FAIL_THRESHOLD:
                entry["alerted"] = True
                message = f"{name} 前端入口连续失败（VPNGate 出口正常，不切换）"
                entry["alert_message"] = message
                set_state(last_alert={"ts": checked_at, "message": message})
                result["alerts"].append(name)
                log_to_json("WARNING", "Frontend", message)
    if probed:
        health = {name: bool(FRONTEND_WATCHDOG_STATE[name]["last_ok"]) for name in names}
        health["checked_at"] = checked_at
        set_state(frontend_health=health)
    return result

def _emit_alert(message: str) -> None:
    """告警写 state.last_alert；配置了 Telegram 凭据则发送，发送失败吞异常不影响状态机。"""
    set_state(last_alert={"ts": time.time(), "message": message})
    ...
    try:
        ...
        print(f"[告警] Telegram 发送失败（已忽略，不影响状态机）: {exc}", flush=True)

def run_failure_attribution() -> dict[str, Any]:
    """故障归因：VPNGate 槽探针连续失败阈值判定 + 前端探针 → classify_failure → 按 action 分发。

    attribution_lock 非阻塞重入保护：collector/checker 并发触发时只有一个执行归因，
    其余拿到 action="skip" 的占位结果，由在执的那次统一决定 failover/pause/hold。
    """
    if not attribution_lock.acquire(blocking=False):
        return {
            "attribution": "attribution-in-progress",
            "action": "skip",
            ...
            "active_slot": None,
            "vpngate_probe_detail": "并发归因已在执行中，本次跳过",
        }
    try:
        return _run_failure_attribution_locked()
    finally:
        attribution_lock.release()

def _run_failure_attribution_locked() -> dict[str, Any]:
    checked_at = time.time()
    active_slot = current_active_slot()
    vpngate_ok = False
    probe_detail = "no-active-slot"
    if active_slot is not None:
        for attempt in range(max(1, VPNGATE_PROBE_FAIL_THRESHOLD)):
            probe = health_probes.probe_vpngate_slot(
                active_slot,
                str(DATA_DIR),
                baseline_ip=direct_public_ip(),
                process_alive=_slot_process_alive,
            ...
            if attempt + 1 < VPNGATE_PROBE_FAIL_THRESHOLD:
                time.sleep(VPNGATE_PROBE_INTERVAL_SECONDS)
    frontend_cfg = _load_frontend_probe_config()
    vless_ok = _frontend_probe_consecutive("vless", frontend_cfg.get("vless"))
    trojan_ok = _frontend_probe_consecutive("trojan", frontend_cfg.get("trojan"))
    classification = health_probes.classify_failure(vpngate_ok, vless_ok, trojan_ok)
    classification["checked_at"] = checked_at
    classification["active_slot"] = active_slot
    classification["vpngate_probe_detail"] = probe_detail

    alert = classification.get("alert")
    if alert:
        _emit_alert(str(alert))
    action = classification.get("action")
    if action == "failover":
        classification["failover_ok"] = failover_to_standby()
    elif action == "pause_rotation":
        set_state(rotation_paused=True)
    if classification.get("pause_performance_switch"):
        set_state(performance_switch_paused=True)
    log_to_json(
        "INFO", "Attribution",
        f"故障归因: {classification['attribution']} action={action} vpngate_ok={vpngate_ok}",
    )
    return classification

def handle_active_tunnel_loss() -> dict[str, Any] | None:
    """active 槽进程死亡/隧道失效的统一第一响应入口。

    所有 `not active_openvpn_running()` 触发点（collector_loop、maintain_valid_nodes）
    必须先走本函数：由 run_failure_attribution 按故障矩阵决定 failover/pause/hold
    （failover 优先 promote 已验证 standby 槽，记节点失败 6h 冷却与切换记录）。
    auto_switch_node 同节点重连只作为"无可用 standby 且无活动隧道"的兜底，
    且矩阵判定 host-stack（pause_rotation）时不兜底；并发触发经 attribution_lock
    只执行一次归因，并发跳过（action="skip"）时不重复兜底。
    """
    ui_cfg = load_ui_config()
    ...
    if ui_cfg.get("routing_mode", "auto") == "fixed_ip":
        return None
    classification = run_failure_attribution()
    action = classification.get("action")
    if action == "skip":
        return classification
    if not active_openvpn_running() and action != "pause_rotation":
        log_to_json(
            "WARNING", "Failover",
            f"归因 action={action} 后仍无活动隧道，执行 auto_switch_node 兜底重连",
        )
        auto_switch_node()
    return classification

def _recover_slots_on_boot() -> None:
    """启动恢复：不信任 slot_states.json，逐槽重建隧道并实测出口后才恢复活动指针；两槽皆败保持无指针。"""
    saved = slot_state.read_slot_states(str(DATA_DIR))
    pointer = slot_state.read_active_slot(str(DATA_DIR))
    slot_state.clear_active_slot(str(DATA_DIR))
    if not isinstance(saved, dict) or not saved:
        return
    preferred_slot = str(pointer["slot"]) if pointer else ""
    ...
    for slot in SLOT_RUNTIME:
        ...
        if not node_id:
            continue
        try:
            connect_node(node_id, slot=slot)
            egress = verify_slot_egress(slot)
        except Exception as exc:
            log_to_json("WARNING", "Boot", f"槽位 {slot} 隧道重建失败: {exc}")
            egress = {"ok": False, "error": str(exc)}
        if not egress.get("ok"):
            log_to_json("WARNING", "Boot", f"槽位 {slot} 出口复验失败: {egress.get('error')}")
            try:
                stop_slot_openvpn(slot)
            except Exception:
                pass
            continue
        recovered[slot] = node_id
    ordered = ([preferred_slot] if preferred_slot in recovered else []) + [
        slot for slot in recovered if slot != preferred_slot
    ]
    for slot in ordered:
        pointer_now = slot_state.read_active_slot(str(DATA_DIR))
        if pointer_now and pointer_now.get("slot") == slot:
            # connect_node 已成功接管该槽，无需重复 promote
            set_state(fail_closed=False, fail_closed_reason="")
            return
        try:
            promote_slot_to_active(slot, recovered[slot])
        except RuntimeError as exc:
            log_to_json("WARNING", "Boot", f"槽位 {slot} 提升失败: {exc}")
            continue
        set_state(fail_closed=False, fail_closed_reason="")
        return
    if recovered:
        log_to_json("ERROR", "Boot", "已验证槽位提升全部失败，保持无活动指针")
    elif saved:
        log_to_json("WARNING", "Boot", "全部槽位重建验证失败，保持无活动指针（fail-closed）")

def sort_all_nodes(nodes: list[dict[str, Any]]) -> list[dict[str, Any]]:
    pool_order = {"trusted": 0, "observation": 1, "candidate": 2, "cooldown": 3, "dormant": 4}
    available_nodes = sorted(
        [n for n in nodes if n.get("probe_status") == "available" or n.get("active")],
        key=lambda n: (
            pool_order.get(str(n.get("pool_state") or "candidate"), 9),
            -parse_int(n.get("composite_score")),
            -parse_int(n.get("ip_quality_score")),
...
def apply_routing_filters(
    ...
    if routing_ip_type == "residential":
        accepted_types = ("residential",) if STRICT_RESIDENTIAL_ONLY else ("residential", "mobile")
        candidates = [
            n for n in candidates
            if n.get("ip_type") in accepted_types
...
def strict_residential_policy_enabled(ui_cfg: dict[str, Any]) -> bool:
    return STRICT_RESIDENTIAL_ONLY and ui_cfg.get("routing_ip_type", "all") == "residential"
...
def standby_nodes(
    ...
    if is_upstream_consumer():
        try:
            snapshot = load_upstream_snapshot(now=now)
        except pool_snapshot.SnapshotValidationError:
            return []
        checked_at = time.time() if now is None else now
        allowed = upstream_pool.select_connectable_nodes(
            snapshot,
            upstream_pool.load_probe_state(LOCAL_PROBE_STATE_FILE),
            checked_at,
        )
        allowed = upstream_pool.rank_local_nodes(
            allowed,
            now=checked_at,
            speed_gain_percent=SPEED_SWITCH_GAIN_PERCENT,
        )
        ...
        return [
            by_id[item["id"]]
            for item in allowed
            if item["id"] in by_id and not by_id[item["id"]].get("active")
        ]
    checked_at = time.time() if now is None else now
    candidates = [
        ...
        and node.get("pool_state") in {"trusted", "observation"}
        and checked_at - float(node.get("probed_at", 0) or 0) <= STANDBY_MAX_AGE_SECONDS
    ]
    candidates = apply_routing_filters(candidates, ui_cfg)
    candidates.sort(key=node_pool.rank_key)
    return candidates

def update_standby_pool_state(ui_cfg: dict[str, Any] | None = None) -> dict[str, Any]:
    cfg = ui_cfg or load_ui_config()
    ready_nodes = standby_nodes(read_nodes(), cfg)
    ready_ids = [str(node.get("id") or "") for node in ready_nodes if node.get("id")]
    shortage = max(0, STANDBY_TARGET - len(ready_ids))
    snapshot = {
        "strict_residential_only": STRICT_RESIDENTIAL_ONLY,
        "standby_target": STANDBY_TARGET,
        "standby_ready": len(ready_ids),
        "standby_shortage": shortage,
        "standby_node_ids": ready_ids[:STANDBY_TARGET],
    }
    set_state(**snapshot)
    return snapshot

def enter_fail_closed(reason: str, ui_cfg: dict[str, Any] | None = None) -> None:
    cfg = ui_cfg or load_ui_config()
    try:
        slot_state.clear_active_slot(str(DATA_DIR))
    except Exception as exc:
        log_to_json("WARNING", "FailClosed", f"清除活动槽指针失败: {exc}")
    stop_active_openvpn()
    with lock:
        nodes = read_nodes()
        for item in nodes:
    ...
    set_state(
        ...
        last_check_message=reason,
    )
    update_standby_pool_state(cfg)

def fill_standby_pool(ui_cfg: dict[str, Any] | None = None) -> dict[str, Any]:
    if is_upstream_consumer():
        refresh_upstream_pool_state()
        return update_standby_pool_state(ui_cfg)
    cfg = ui_cfg or load_ui_config()
    snapshot = update_standby_pool_state(cfg)
    if snapshot["standby_shortage"] <= 0:
        return snapshot

    tested = 0
    while tested < STANDBY_SCAN_LIMIT:
        with lock:
            nodes = read_nodes()
            fresh_ids = {str(node.get("id")) for node in standby_nodes(nodes, cfg)}
            candidates = [
                node for node in nodes
                if node.get("id")
                and not node.get("active")
                and str(node.get("id")) not in fresh_ids
                and (
                    node.get("probe_status") != "unavailable"
                    or time.time() - float(node.get("probed_at", 0) or 0) >= INVALID_BACKOFF_SECONDS
                )
            ]
            candidates = apply_routing_filters(candidates, cfg, include_unknown_ip_type=True)
            candidates.sort(key=probe_priority_key)
            batch_ids = [
                str(node["id"]) for node in candidates[:min(STANDBY_SCAN_BATCH_SIZE, STANDBY_SCAN_LIMIT - tested)]
            ]
        if not batch_ids:
            break
        test_multiple_nodes(batch_ids)
        tested += len(batch_ids)
        snapshot = update_standby_pool_state(cfg)
        if snapshot["standby_shortage"] <= 0:
            break

    msg = (
        f"住宅备用池就绪 {snapshot['standby_ready']}/{snapshot['standby_target']}"
        if snapshot["standby_shortage"] <= 0
        else f"住宅备用池不足 {snapshot['standby_ready']}/{snapshot['standby_target']}，后台将继续补齐"
    )
    print(f"[住宅备用池] {msg}", flush=True)
    log_to_json("INFO" if snapshot["standby_shortage"] <= 0 else "WARNING", "Standby", msg)
    return snapshot

def maintain_standby_slot(now: float | None = None) -> bool:
    """standby 槽隧道维护：活动指针有效且 active 槽健康时，为空闲槽建热备隧道。

    补"槽"（本函数，consumer 模式同样可用）与补"池"（schedule_standby_replenish
    的节点池填充）分离。候选复用 standby_nodes 的池约束并排除任何已占用槽的节点；
    connect_node 内部 verify_slot_egress 校验出口 ≠ 直连基线且 ≠ 另一槽出口，
    成功后 _adopt_slot_after_connect 保证不抢占健康 active 的活动指针。
    空闲判定认 process.poll()：process 为 None 或已退出（含外部 kill 的残留对象）
    即为空闲，残留由 connect_node 内的 stop_slot_openvpn 统一清理。
    同一槽连续 STANDBY_SLOT_MAX_FAILURES 次失败后暂停 STANDBY_SLOT_PAUSE_SECONDS，
    失败节点经 _record_node_failure 计入既有失败冷却；任一时刻最多两个 OpenVPN 进程。
    """
    checked_at = time.time() if now is None else now
    if is_connecting:
        return False
    try:
        ...
        return False
    if not isinstance(pointer, dict) or pointer.get("slot") not in SLOT_RUNTIME:
        return False
    active_slot = str(pointer["slot"])
    active_runtime = SLOT_RUNTIME[active_slot]
    active_process = active_runtime["process"]
    active_node_id = str(active_runtime["node_id"] or "")
    if active_process is None or not active_node_id:
        return False
    try:
    ...
    try:
        ...
        saved = {}
    active_saved = saved.get(active_slot) if isinstance(saved, dict) else None
    if not isinstance(active_saved, dict) or str(active_saved.get("health") or "") != "ok":
        return False
    idle_slots = [
        slot
        for slot, runtime in SLOT_RUNTIME.items()
        if slot != active_slot
        and (_slot_process_dead(runtime) or not runtime["node_id"])
        and runtime["draining_since"] is None
    ]
    if not idle_slots:
        return False
    target_slot = idle_slots[0]
    if checked_at < float(standby_slot_paused_until.get(target_slot, 0.0) or 0.0):
        return False
    if not standby_slot_lock.acquire(blocking=False):
        return False
    try:
        if is_connecting:
            return False
        try:
            candidates = standby_nodes(read_nodes(), load_ui_config(), now=checked_at)
        except Exception as exc:
            log_to_json("WARNING", "Standby", f"读取 standby 候选失败: {exc}")
            return False
        occupied_ids = {
            str(runtime["node_id"] or "")
            for runtime in SLOT_RUNTIME.values()
            if runtime["node_id"] and not _slot_process_dead(runtime)
        }
        node_id = ""
        for candidate in candidates:
            candidate_id = str(candidate.get("id") or "")
            if candidate_id and candidate_id not in occupied_ids:
                node_id = candidate_id
                break
        if not node_id:
            return False
        try:
            connect_node(node_id, slot=target_slot)
        except Exception as exc:
            failures = int(standby_slot_failures.get(target_slot, 0)) + 1
            standby_slot_failures[target_slot] = failures
            _record_node_failure(node_id)
            if failures >= STANDBY_SLOT_MAX_FAILURES:
                standby_slot_paused_until[target_slot] = time.time() + STANDBY_SLOT_PAUSE_SECONDS
                standby_slot_failures[target_slot] = 0
                log_to_json(
                    "WARNING", "Standby",
                    f"standby 槽位 {target_slot} 连续 {failures} 次建备失败，暂停 {STANDBY_SLOT_PAUSE_SECONDS} 秒后重试",
                )
            else:
                log_to_json(
                    "WARNING", "Standby",
                    f"standby 槽位 {target_slot} 建备失败（第 {failures} 次）: {exc}",
                )
            return False
        standby_slot_failures[target_slot] = 0
        standby_slot_paused_until[target_slot] = 0.0
        log_to_json(
            "INFO", "Standby",
            f"standby 槽位 {target_slot} 已建立热备隧道（节点 {node_id}），活动指针保持 {active_slot}",
        )
        return True
    finally:
        standby_slot_lock.release()

def schedule_standby_replenish() -> None:
    """补备入口：先补"槽"（为空闲 standby 槽建隧道，consumer 模式同样可用），再补"池"。"""
    try:
        maintain_standby_slot()
    except Exception as exc:
        log_to_json("WARNING", "Standby", f"standby 槽补备异常: {exc}")
    if is_upstream_consumer():
        return
    cfg = load_ui_config()
    if not strict_residential_policy_enabled(cfg):
        return
    snapshot = update_standby_pool_state(cfg)
    if snapshot["standby_shortage"] <= 0 or not standby_replenish_lock.acquire(blocking=False):
        return
        ...
        try:
            for _ in range(12):
                ...
                if maintenance_lock.locked():
                    continue
                latest = update_standby_pool_state(load_ui_config())
                if latest["standby_shortage"] <= 0:
                    return
                maintain_valid_nodes(force=False)
                return
        finally:
            standby_replenish_lock.release()
...
def validate_node_allowed_by_routing(node: dict[str, Any], ui_cfg: dict[str, Any]) -> None:
    ...
    accepted_residential_types = ("residential",) if STRICT_RESIDENTIAL_ONLY else ("residential", "mobile")
    if routing_ip_type == "residential" and node_ip_type not in accepted_residential_types:
...
def test_config_path(node_id: str) -> Path:
    ...
    return TEMP_DIR / f"test_{safe_id}_{uuid.uuid4().hex}.ovpn"

def direct_public_ip() -> str:
    for url in ("https://api.ipify.org", "https://ip.sb"):
        try:
            result = subprocess.run(
                ["curl", "-4", "-fsS", "--connect-timeout", "5", "--max-time", "10", url],
                capture_output=True, text=True, timeout=12,
            )
            value = result.stdout.strip()
            if result.returncode == 0 and re.fullmatch(r"\d+\.\d+\.\d+\.\d+", value):
                return value
        except Exception:
            pass
    return ""

def lightweight_probe_ok(egress: dict[str, Any], baseline_ip: str) -> bool:
    exit_ip = str(egress.get("exit_ip") or "")
    return bool(
        egress.get("ok")
        and exit_ip
        and baseline_ip
        and exit_ip != baseline_ip
    )

def lightweight_probe_node_by_id(node_id: str) -> dict[str, Any]:
    if not is_upstream_consumer():
        raise RuntimeError("轻量复核只允许在 upstream-consumer 模式运行")
    nodes = read_nodes()
    node = next((item for item in nodes if item.get("id") == node_id), None)
    if not node:
        raise ValueError(f"Node not found: {node_id}")
    validate_node_allowed_by_routing(node, load_ui_config())
    digest = str(node.get("config_hash") or "")
    state = upstream_pool.load_probe_state(LOCAL_PROBE_STATE_FILE)
    if upstream_pool.probe_is_current(state, node_id, digest) and float(
        state.get(node_id, {}).get("local_measured_at", 0) or 0
    ) > 0:
        return node

    baseline = direct_public_ip()
    if not baseline:
        raise RuntimeError("无法取得云途默认公网出口基线，拒绝复核以防止出口回落")
    ...
    try:
        ...
        if ok and process is not None:
            ...
            egress = vpn_utils.probe_tunnel_egress(
                dev_name,
                download_bytes=UPSTREAM_LOCAL_PROBE_BYTES,
                upload_bytes=0,
                samples=UPSTREAM_LOCAL_PROBE_SAMPLES,
            )
            success = lightweight_probe_ok(egress, baseline)
            record_speed_probe_traffic(
                int(egress.get("probe_bytes_down", 0) or 0),
                int(egress.get("probe_bytes_up", 0) or 0),
                str(egress.get("probe_path") or "isolated-vpngate-ipv4-cloudflare"),
            )
            message = (
                f"云途单连接测速通过，隧道出口 {egress.get('exit_ip')}，下载 {egress.get('download_mbps', 0)} Mbps"
                if success
                else f"隧道请求失败或出口回落: {egress.get('error', '出口与云途公网相同')}"
            )
    finally:
        stop_process(process)
        if idx is not None:
    ...
    state = upstream_pool.record_probe_result(
        state, node_id, digest, success, error_class, time.time(),
        UPSTREAM_PROBE_COOLDOWN_SECONDS,
        local_download_mbps=float(egress.get("download_mbps", 0) or 0) if success else None,
        local_ttfb_ms=int(egress.get("ttfb_ms", 0) or 0) if success else 0,
        local_probe_bytes=int(egress.get("probe_bytes_total", 0) or 0) if success else 0,
        local_probe_path=str(egress.get("probe_path") or "") if success else "",
    )
    write_json(LOCAL_PROBE_STATE_FILE, state)
    refresh_upstream_pool_state()
    if not success:
        raise RuntimeError(message or "云途轻量复核失败")
    return next((item for item in read_nodes() if item.get("id") == node_id), node)


def maintain_upstream_consumer_pool(now: float | None = None) -> dict[str, Any]:
    if not is_upstream_consumer():
        raise RuntimeError("upstream consumer maintenance requires upstream-consumer mode")
    checked_at = time.time() if now is None else float(now)
    pools = refresh_upstream_pool_state()
    nodes = read_nodes()
    probe_state = upstream_pool.load_probe_state(LOCAL_PROBE_STATE_FILE)
    queue_ids = upstream_pool.probe_queue_ids(
        nodes,
        limit=UPSTREAM_LOCAL_PROBE_BATCH_SIZE,
        now=checked_at,
        probe_state=probe_state,
    )
    results = test_multiple_nodes(
        queue_ids,
        UPSTREAM_LOCAL_PROBE_BYTES,
        0,
        UPSTREAM_LOCAL_PROBE_SAMPLES,
    ) if queue_ids else []
    pools = refresh_upstream_pool_state()
    set_state(
        last_upstream_probe_at=checked_at,
        last_upstream_probe_count=len(queue_ids),
        last_upstream_probe_ids=queue_ids,
        local_throughput_ready=sum(
            1 for node in read_nodes() if float(node.get("local_measured_at", 0) or 0) > 0
        ),
    )
    return {"pools": pools, "queued_ids": queue_ids, "results": results}
...
def evaluate_test_result(
    ...
    latency: int,
    download_bytes: int = FULL_PROBE_BYTES,
    upload_bytes: int = 0,
    samples: int = 1,
) -> dict[str, Any]:
    result = {
    ...
    for key in (
        "first_seen_at", "success_count", "failure_count", "consecutive_successes",
        "consecutive_failures", "failure_days", "last_success_at", "last_failure_at",
    ):
        if key in node:
            result[key] = node[key]
    try:
        if not ok or process is None:
            node_pool.record_probe(result, False)
            return result
        egress = vpn_utils.probe_tunnel_egress(
            dev_name, download_bytes=download_bytes, upload_bytes=upload_bytes, samples=samples,
        )
        record_speed_probe_traffic(
            int(egress.get("probe_bytes_down", 0) or 0),
            int(egress.get("probe_bytes_up", 0) or 0),
        ...
        if not egress.get("ok"):
            result["probe_message"] = f"隧道已建立，但实际出口验证失败: {egress.get('error', '未知错误')}"
            node_pool.record_probe(result, False)
            return result
        exit_ip = str(egress.get("exit_ip") or "")
        reputation = vpn_utils.score_ip_reputation(exit_ip)
        exit_info = {"ip": exit_ip, "remote_host": exit_ip, "owner": "", "asn": "", "as_name": "", "location": "", "ip_type": "", "quality": ""}
        vpn_utils.enrich_ip_info([exit_info])
        actual_type = str(reputation.get("ip_type") or "")
        if actual_type != "residential" and exit_info.get("ip_type") == "residential" and not reputation.get("risk_flags"):
            actual_type = "residential"
        actual_country = str(reputation.get("country") or "")
        if not actual_country and "日本" in str(exit_info.get("location") or ""):
            actual_country = "JP"
        result.update(
            actual_exit_ip=exit_ip,
            actual_country=actual_country,
            actual_ip_type=actual_type,
            risk_free=bool(reputation.get("risk_free")),
            ip_quality_score=int(reputation.get("ip_quality_score", 0) or 0),
            risk_flags=reputation.get("risk_flags", []),
            hard_risk_flags=reputation.get("hard_risk_flags", []),
            score_sources=reputation.get("score_sources", []),
            score_mode=reputation.get("score_mode", "fallback"),
            score_components=reputation.get("score_components", {}),
            score_weights=reputation.get("score_weights", {}),
            netcoffee_asn=reputation.get("netcoffee_asn"),
            netcoffee_asn_kind=reputation.get("netcoffee_asn_kind", ""),
            netcoffee_company_type=reputation.get("netcoffee_company_type", ""),
            netcoffee_residential=bool(reputation.get("netcoffee_residential")),
            cleanip_score=reputation.get("cleanip_score"),
            cleanip_grade=reputation.get("cleanip_grade", ""),
            cleanip_confidence=reputation.get("cleanip_confidence", 0),
            cleanip_residential_probability=reputation.get("cleanip_residential_probability", 0),
            cleanip_risk_score=reputation.get("cleanip_risk_score", 0),
            scored_at=reputation.get("scored_at", time.time()),
            download_mbps=float(egress.get("download_mbps", 0) or 0),
            upload_mbps=float(egress.get("upload_mbps", 0) or 0),
            ttfb_ms=int(egress.get("ttfb_ms", 0) or 0),
            speed_samples=int(egress.get("speed_samples", 0) or 0),
            speed_probe_path=egress.get("probe_path", ""),
            speed_probe_bytes=int(egress.get("probe_bytes_total", 0) or 0),
        ...
        eligible = actual_country.upper() in {"JP", "JAPAN", "日本"} and actual_type == "residential" and bool(result["risk_free"])
        result["probe_status"] = "available" if eligible else "unavailable"
        result["probe_message"] = (
            f"实际出口 {exit_ip}；日本住宅；IP质量 {result['ip_quality_score']}；下载 {result['download_mbps']} Mbps"
            if eligible else f"严格拒绝实际出口 {exit_ip}: 国家={actual_country or '未知'} 类型={actual_type or '未知'} 风险={result.get('risk_flags', [])}"
        )
        node_pool.record_probe(result, eligible)
        node_pool.calculate_scores(result)
        return result
    finally:
        stop_process(process)

def stable_pool_probe_ids(nodes: list[dict[str, Any]], now: float | None = None) -> list[str]:
    if STABLE_POOL_PROBE_DAILY_LIMIT <= 0:
        return []
    checked_at = time.time() if now is None else now
    eligible = [
        node for node in nodes
        ...
        and not node.get("active")
        and node.get("pool_state") in {"trusted", "observation"}
        and checked_at - float(node.get("probed_at", 0) or 0) >= 24 * 3600
    ]
    # Daily bandwidth goes to the best quality challengers. Add at most one
    # seven-day-old node so the rest of the bounded pool is still refreshed.
    ranked = sorted(eligible, key=node_pool.rank_key)
    selected = ranked[:STABLE_POOL_PROBE_DAILY_LIMIT]
    weekly = sorted(
        [node for node in eligible if checked_at - float(node.get("probed_at", 0) or 0) >= 7 * 86400 and node not in selected],
        key=lambda node: (float(node.get("probed_at", 0) or 0), str(node.get("id") or "")),
    )
    if weekly:
        selected.append(weekly[0])
    return [str(node["id"]) for node in selected]
...
def test_node_by_id(
    ...
    if is_upstream_consumer():
        return lightweight_probe_node_by_id(node_id)
    with lock:
        nodes = read_nodes()
        node = next((item for item in nodes if item.get("id") == node_id), None)
        if not node:
    ...
    try:
        idx = get_free_test_index()
        dev_name = f"tun{idx}"
        ok, message, process = run_openvpn_until_ready(str(temp_path), keep_alive=True, route_nopull=True, timeout=12, dev=dev_name)
        result = evaluate_test_result(node, dev_name, ok, message, process, latency, download_bytes, upload_bytes, samples)
    finally:
        stop_process(process)
        if idx is not None:
            release_test_index(idx)
    ...
    with lock:
        ...
        if node:
            ...
            refresh_pool_state(sorted_nodes)
...
def test_multiple_nodes(
    ...
    if is_upstream_consumer():
        ...
        for node_id in node_ids:
            try:
                results.append(lightweight_probe_node_by_id(node_id))
            except Exception:
                continue
        return results
    with lock:
    ...
    def test_worker(args: tuple[int, dict[str, Any]]) -> dict[str, Any]:
        ...
        try:
            ...
            ok, message, process = run_openvpn_until_ready(str(temp_path), keep_alive=True, route_nopull=True, timeout=12, dev=dev_name)
            return evaluate_test_result(
                n_info, dev_name, ok, message, process, latency,
                download_bytes, upload_bytes, samples,
            )
        finally:
            stop_process(process)
    ...
    updated_nodes_map = {}
    max_workers = min(TEST_MAX_WORKERS, max(1, len(to_test)))
...
def auto_switch_node(attempt: int = 0) -> None:
    ...
    with lock:
        nodes = read_nodes()
        candidates = standby_nodes(nodes, ui_cfg)
    ...
    if candidates:
        ...
        msg = "没有可用的备选节点，已严格失败关闭"
        if routing_mode == "fixed_region" and target_country:
            msg = f"没有可用的【{target_country}】备选节点，已严格失败关闭"
        print(f"[自动切换] {msg}", flush=True)
        log_to_json("WARNING", "VPN", msg)
        enter_fail_closed(msg, ui_cfg)
        if is_upstream_consumer():
            return
...
def optimization_candidate_reason(current: dict[str, Any], candidate: dict[str, Any]) -> str:
    """Return why a candidate wins while keeping IP quality lexicographically first."""
    current_quality = int(current.get("ip_quality_score", 0) or 0)
    candidate_quality = int(candidate.get("ip_quality_score", 0) or 0)
    quality_gain = candidate_quality - current_quality
    if quality_gain > QUALITY_TIE_WINDOW:
        return "weighted_ip_quality"
    if quality_gain < -QUALITY_TIE_WINDOW:
        return ""
    current_speed = float(current.get("download_mbps", 0) or 0)
    candidate_speed = float(candidate.get("download_mbps", 0) or 0)
    required_speed = current_speed * (1 + SPEED_SWITCH_GAIN_PERCENT / 100)
    if current_speed > 0 and candidate_speed >= required_speed:
        return "quality_close_speed_gain"
    return ""


def maybe_switch_to_better_node() -> bool:
    """Perform at most one maintenance-driven optimization per local day."""
    global is_connecting
    if not active_openvpn_running() or not active_openvpn_node_id:
        return False
    state = get_state()
    if float(state.get("switch_cooldown_until", 0) or 0) > time.time():
        return False
    if state.get("rotation_paused") or state.get("performance_switch_paused"):
        return False
    today = time.strftime("%Y-%m-%d", time.localtime())
    if state.get("last_optimization_switch_date") == today:
        return False
    nodes = read_nodes()
    current = next((node for node in nodes if node.get("id") == active_openvpn_node_id), None)
    if not current:
        return False
    candidates = standby_nodes(nodes, load_ui_config())
    if not candidates:
        return False
    best = candidates[0]
    reason = optimization_candidate_reason(current, best)
    if not reason:
        set_state(optimization_candidate_id="", optimization_candidate_wins=0, optimization_candidate_reason="")
        return False
    if time.time() - float(best.get("probed_at", 0) or 0) > 6 * 3600:
        return False
    best_id = str(best.get("id") or "")
    previous_id = str(state.get("optimization_candidate_id") or "")
    wins = int(state.get("optimization_candidate_wins", 0) or 0) + 1 if previous_id == best_id else 1
    set_state(
        optimization_candidate_id=best_id,
        optimization_candidate_wins=wins,
        optimization_candidate_reason=reason,
        optimization_candidate_checked_at=time.time(),
    )
    if wins < SWITCH_REQUIRED_WINS:
        return False
    previous_connecting = is_connecting
    is_connecting = False
    try:
        connect_node(best_id, manual=False)
        set_state(
            last_optimization_switch_date=today, manual_override=False,
            optimization_candidate_id="", optimization_candidate_wins=0,
            last_optimization_reason=reason,
        )
        return True
    except Exception as exc:
        log_to_json("WARNING", "Pool", f"择优切换失败，保留失败关闭策略并等待故障恢复: {exc}")
        return False
    finally:
        is_connecting = previous_connecting

def connect_node(node_id: str, manual: bool = False, slot: str = "A") -> str:
    global is_connecting
    if slot not in SLOT_RUNTIME:
        raise ValueError(f"invalid slot: {slot!r}")
    device = slot_state.SLOT_DEVICES[slot]
    table = slot_state.SLOT_TABLES[slot]
    node_id = str(node_id or "").strip()
    if not node_id:
    ...
    try:
        ...
        if is_upstream_consumer():
            probe_state = upstream_pool.load_probe_state(LOCAL_PROBE_STATE_FILE)
            if not upstream_pool.probe_is_current(
                probe_state, node_id, str(node.get("config_hash") or "")
            ):
                lightweight_probe_node_by_id(node_id)
                nodes = read_nodes()
                node = next((item for item in nodes if item.get("id") == node_id), node)
            if not direct_public_ip():
                raise RuntimeError("无法取得云途默认公网出口基线，拒绝建立活动连接")
        ...
        set_state(active_node_latency="清理连接", last_check_message=f"正在关闭与清理槽位 {slot} 的旧的 VPN 连接及网卡...")
        stop_slot_openvpn(slot)
        ...
        with lock:
            SLOT_RUNTIME[slot]["process"] = process
            SLOT_RUNTIME[slot]["node_id"] = node_id
            SLOT_RUNTIME[slot]["draining_since"] = None
        _sync_legacy_active_state()

        set_state(active_node_latency="配置路由", last_check_message="正在配置策略路由规则与流量转发...")
        setup_policy_routing(device, table)
        ...
        set_state(last_check_message=f"正在验证槽位 {slot} 隧道的实测出口 IP...")
        egress = verify_slot_egress(slot)
        if not egress.get("ok"):
            reason = f"候选节点 {node_id} 槽位 {slot} 出口验证失败: {egress.get('error') or '未知错误'}"
            node["probe_status"] = "unavailable"
            ...
            write_json(NODES_FILE, nodes)
            stop_slot_openvpn(slot)
            # 仅当失败槽是当前活动槽（或没有任何活动槽）才进入 fail-closed；
            # standby 槽建备失败不得拆掉仍在服务的活动隧道
            if current_active_slot() in (None, slot):
                enter_fail_closed(reason, ui_cfg)
            else:
                set_state(is_connecting=False, last_check_message=reason)
            raise RuntimeError(reason)
        actual_exit = str(egress.get("exit_ip") or "")
        slot_latency_ms = int(egress.get("ttfb_ms") or 0)
        if is_upstream_consumer():
            node.update(
                actual_exit_ip=actual_exit,
                probe_status="available",
                probe_message="云途活动隧道复核通过；地域、住宅与风险结论继承 BitsFlow",
                probed_at=time.time(),
            )
            probe_state = upstream_pool.record_probe_result(
                upstream_pool.load_probe_state(LOCAL_PROBE_STATE_FILE),
            ...
            set_state(
                proxy_ok=True,
                proxy_ip=actual_exit,
                proxy_latency_ms=slot_latency_ms,
                proxy_error="",
                fail_closed=False,
                fail_closed_reason="",
        ...
        if not is_upstream_consumer():
            reputation = vpn_utils.score_ip_reputation(actual_exit)
            if not reputation.get("score_sources") and node.get("actual_exit_ip") == actual_exit:
                reputation = {
                    "country": node.get("actual_country", ""),
                    "ip_type": node.get("actual_ip_type", ""),
                    "risk_free": node.get("risk_free", False),
                    "ip_quality_score": node.get("ip_quality_score", 0),
                    "risk_flags": node.get("risk_flags", []),
                    "score_sources": node.get("score_sources", []),
                    "scored_at": node.get("scored_at", 0),
                }
            hard_ok = (
                str(reputation.get("country") or "").upper() in {"JP", "JAPAN", "日本"}
                and reputation.get("ip_type") == "residential"
                and bool(reputation.get("risk_free"))
            )
            if not hard_ok:
                reason = (
                    f"实际出口 {actual_exit or '未知'} 未通过严格日本住宅检查: "
                    f"国家={reputation.get('country') or '未知'} 类型={reputation.get('ip_type') or '未知'} "
                    f"风险={reputation.get('risk_flags', [])}"
                )
                node["probe_status"] = "unavailable"
                node["probe_message"] = reason
                node_pool.record_probe(node, False)
            ...
            node.update(
                actual_exit_ip=actual_exit,
                actual_country=reputation.get("country", ""),
                actual_ip_type=reputation.get("ip_type", ""),
                risk_free=True,
                ip_quality_score=int(reputation.get("ip_quality_score", 0) or 0),
                risk_flags=reputation.get("risk_flags", []),
                hard_risk_flags=reputation.get("hard_risk_flags", []),
                score_sources=reputation.get("score_sources", []),
                score_mode=reputation.get("score_mode", "weighted_fallback"),
                score_components=reputation.get("score_components", {}),
                score_weights=reputation.get("score_weights", {}),
                netcoffee_asn=reputation.get("netcoffee_asn"),
                netcoffee_asn_kind=reputation.get("netcoffee_asn_kind", ""),
            ...
            node_pool.record_probe(node, True)
            node_pool.calculate_scores(node)
            write_json(NODES_FILE, nodes)
            set_state(
                proxy_ok=True,
                proxy_ip=actual_exit,
        ...
        slot_states = slot_state.read_slot_states(str(DATA_DIR))
        slot_states[slot] = {
            "node_id": node_id,
            "device": device,
            "exit_ip": actual_exit,
            "asn": str(node.get("asn") or node.get("netcoffee_asn") or ""),
            "verified_at": time.time(),
            "health": "ok",
        }
        slot_state.write_slot_states(str(DATA_DIR), slot_states)
        _adopt_slot_after_connect(slot, node_id)
        ...
        refresh_pool_state(nodes)
        update_standby_pool_state(ui_cfg)
        log_to_json("INFO", "VPN", f"节点 {node_id} 连接成功，槽位 {slot} 出口网卡 {device} 已启用")
        return f"Connected {node_id}"
    except Exception as exc:
        # 仅当失败发生在活动槽（或没有任何活动槽）时才清理活动连接状态；
        # standby 槽建备/切换候选失败不得拆掉仍在服务的活动隧道
        if current_active_slot() in (None, slot) or (
            active_openvpn_node_id == node_id and not active_openvpn_running()
        ):
    ...
    try:
        if force:
            ...
            if connection_enabled:
                if routing_mode == "fixed_ip":
                    ...
                    print("[维护线程] 检测到当前 OpenVPN 进程已意外退出，进入故障归因流程", flush=True)
                    is_connecting = False
                    try:
                        handle_active_tunnel_loss()
                    finally:
                        is_connecting = True
        ...
        with lock:
            ...
            for cand in candidates:
                if cand["id"] not in seen_ids:
                    ...
                    node_pool.initialize_node(cand)
                    cand["last_seen_at"] = float(cand.get("fetched_at", time.time()) or time.time())
                    merged.append(cand)
            ...
            managed_ids = node_pool.managed_config_ids(CONFIG_DIR)
            retained_ids = {str(item.get("id")) for item in merged if item.get("id")}
            try:
                node_pool.prune_managed_configs(CONFIG_DIR, managed_ids, retained_ids)
            except OSError as exc:
                log_to_json("WARNING", "Pool", f"清理过期节点配置失败: {exc}")
        ...
        if should_fast_connect:
            with lock:
                ...
                fast_candidates.sort(key=lambda node: (
                    0 if str(node.get("id") or "") == startup_preferred_node_id else 1,
            ...
            if fast_test_ids:
                ...
                test_multiple_nodes(fast_test_ids, FULL_PROBE_BYTES)
                ...
                if available_candidates:
                    ...
                    if active_openvpn_running():
                        standby_snapshot = fill_standby_pool(ui_cfg) if strict_residential_policy_enabled(ui_cfg) else update_standby_pool_state(ui_cfg)
                        valid_nodes_count = len([n for n in read_nodes() if n.get("probe_status") == "available"])
                        message = (
                            f"Fetched {len(candidates)} nodes. Fast-tested {len(fast_test_ids)} nodes and connected. "
                            f"Standby {standby_snapshot['standby_ready']}/{standby_snapshot['standby_target']}."
                        )
                        set_state(
        ...
        if active_openvpn_running() and strict_residential_policy_enabled(ui_cfg):
            active_health = check_proxy_health()
            if active_health.get("ok") and active_openvpn_node_id:
                actual_exit = str(active_health.get("ip") or "")
                reputation = vpn_utils.score_ip_reputation(actual_exit)
                active_speed = vpn_utils.probe_tunnel_egress(
                    "tun0",
                    download_bytes=STABLE_POOL_PROBE_BYTES,
                    upload_bytes=STABLE_POOL_PROBE_UPLOAD_BYTES,
                    samples=STABLE_POOL_PROBE_SAMPLES,
                )
                record_speed_probe_traffic(
                    int(active_speed.get("probe_bytes_down", 0) or 0),
                    int(active_speed.get("probe_bytes_up", 0) or 0),
                    str(active_speed.get("probe_path") or "isolated-vpngate-ipv4-cloudflare"),
                )
                with lock:
                    active_nodes = read_nodes()
                    active_item = next((item for item in active_nodes if item.get("id") == active_openvpn_node_id), None)
                    if active_item:
                        if reputation.get("score_sources"):
                            active_item.update(
                                actual_exit_ip=actual_exit,
                                actual_country=reputation.get("country", ""),
                                actual_ip_type=reputation.get("ip_type", ""),
                                ...
                                cleanip_risk_score=reputation.get("cleanip_risk_score", 0),
                                scored_at=reputation.get("scored_at", time.time()),
                                download_mbps=float(active_speed.get("download_mbps", 0) or 0),
                                upload_mbps=float(active_speed.get("upload_mbps", 0) or 0),
                                ttfb_ms=int(active_speed.get("ttfb_ms", 0) or 0),
                                speed_samples=int(active_speed.get("speed_samples", 0) or 0),
                                speed_probe_path=active_speed.get("probe_path", ""),
                                speed_probe_bytes=int(active_speed.get("probe_bytes_total", 0) or 0),
                                probe_status="available",
                                probed_at=time.time(),
                            )
                            node_pool.record_probe(active_item, node_pool.is_hard_eligible(active_item))
                            write_json(NODES_FILE, active_nodes)
            with lock:
                pool_ids = stable_pool_probe_ids(read_nodes())
            if pool_ids:
                test_multiple_nodes(
                    pool_ids,
                    STABLE_POOL_PROBE_BYTES,
                    STABLE_POOL_PROBE_UPLOAD_BYTES,
                    STABLE_POOL_PROBE_SAMPLES,
                )
            run_daily_full_path_speedtest()
            standby_snapshot = fill_standby_pool(ui_cfg)
            maybe_switch_to_better_node()
            merged = read_nodes()
            valid_nodes_count = len([n for n in merged if n.get("probe_status") == "available"])
            message = (
                f"Fetched {len(candidates)} nodes. Strict residential active; "
                f"standby {standby_snapshot['standby_ready']}/{standby_snapshot['standby_target']}."
            )
            set_state(
                last_check_at=time.time(),
        ...
        update_standby_pool_state(load_ui_config())
        return message
    except Exception as e:
        raise e
    finally:
...
def collector_loop() -> None:
    ...
    while True:
        ...
        if is_upstream_consumer():
            try:
                now = time.time()
                consumer_state = get_state()
                last_probe = float(consumer_state.get("last_upstream_probe_at", 0) or 0)
                if now - last_probe >= UPSTREAM_LOCAL_PROBE_INTERVAL_SECONDS:
                    maintain_upstream_consumer_pool(now)
                else:
                    refresh_upstream_pool_state()
                if not active_openvpn_running() and load_ui_config().get("connection_enabled", True):
                    handle_active_tunnel_loss()
            except Exception as exc:
                set_state(
                    upstream_snapshot_error=str(exc),
                    last_check_message=f"上游快照不可用: {exc}",
                )
                if not active_openvpn_running():
                    enter_fail_closed(f"上游快照不可用于新连接: {exc}")
            time.sleep(60)
            continue
        now = time.time()
        ...
        state = get_state()
        daily_due = (
            DAILY_MAINTENANCE_START_HOUR <= local.tm_hour < DAILY_MAINTENANCE_END_HOUR
            and state.get("last_daily_maintenance_date") != today
        )
        trusted_count = len(state.get("trusted_pool_ids", []) or [])
        observation_count = len(state.get("observation_pool_ids", []) or [])
        last_emergency = float(state.get("last_emergency_replenish_at", 0) or 0)
        emergency_due = (
            (not active_openvpn_running() or observation_count < 3 or trusted_count == 0)
            and now - last_emergency >= EMERGENCY_REFILL_COOLDOWN_SECONDS
        )
        if not daily_due and not emergency_due:
            time.sleep(60)
            continue
        try:
            reason = "每日 03:00-04:00 维护" if daily_due else "节点池不足紧急补池"
            print(f"[守护线程] 开始执行{reason}...", flush=True)
            log_to_json("INFO", "Main", f"开始执行{reason}...")
            res = maintain_valid_nodes(force=False)
            updates: dict[str, Any] = {"last_check_at": time.time()}
            if daily_due:
                updates["last_daily_maintenance_date"] = today
            else:
                updates["last_emergency_replenish_at"] = time.time()
            set_state(**updates)
            log_to_json("INFO", "Main", f"{reason}完成，结果: {res}")
        except Exception as exc:
            err_msg = f"节点池维护任务执行异常: {exc}"
  ...
  <div class="btn-group">
    ...
    <div class="dropdown">
      ...
      <div id="github_dropdown" class="dropdown-content">
        <a href="#" target="_blank">公开仓库（PUBLIC_REPOSITORY_URL）</a>
        <a href="#" target="_blank">测试仓库（PUBLIC_TEST_REPOSITORY_URL）</a>
      </div>
    </div>
    <a href="#" target="_blank" class="btn-telegram">
    ...
    <div style="display: flex; flex-direction: column; gap: 16px;">
      ...
      <div style="border-top: 1px solid rgba(255,255,255,0.06); padding-top: 16px;">
        <div style="padding: 10px 14px; background: rgba(245, 158, 11, 0.1); border: 1px solid rgba(245, 158, 11, 0.25); border-radius: 8px; font-size: 12px; color: var(--warning); line-height: 1.5;">
          <strong>运行规则：</strong>正常落地始终按可信池排序；人工切换失败后自动回到可信池，收藏或取消收藏不会改变节点池状态。
        </div>
      </div>
    </div>
  </div>
    ...
    <div class="modal-content">
      ...
      <form id="credentials_form" onsubmit="saveCredentials(event)">
        ...
        <div class="form-group" style="margin-bottom: 20px;">
          ...
          <input type="text" id="cred_suffix" class="input-field" required pattern="[A-Za-z0-9]+" placeholder="SECRET_PATH">
        </div>
    ...
    <div class="modal-content" style="max-width: 640px;">
      ...
      <div class="vps-links">
        <div class="vps-item">
          ...
          <a href="#" target="_blank" class="vps-btn">VPS_PROVIDER_URL</a>
        </div>
        <div class="vps-item">
      ...
      <div class="vps-footer" style="margin-top: 20px;">
        官方技术支持及优质资源交流论坛：<span class="forum-link">SUPPORT_FORUM_URL</span>
      </div>
      ...
      <div class="vps-footer" style="margin-top: 16px; border-top: 1px solid rgba(255,255,255,0.06); padding-top: 16px; text-align: left; font-size: 13px; color: var(--text-secondary); line-height: 1.6;">
        ...
        <div style="font-family: monospace; background: rgba(0,0,0,0.2); padding: 8px 12px; border-radius: 6px; margin-top: 6px; word-break: break-all; select-all: true;">
          <span style="color: var(--primary); font-weight: bold;">BNB (BSC):</span> DONATION_ADDRESS<br>
          <span style="color: var(--primary); font-weight: bold;">TRX (TRC20):</span> DONATION_ADDRESS
        </div>
      </div>
    </div>
  </div>
...
function getFilteredNodes() {
  ...
  return nodes.filter(n => {
    ...
    if (selectedIpType) {
      if (selectedIpType === "residential" && n.ip_type !== "residential") {
        return false;
      }
...
function render(){
  ...
  const trustedInfo = `${(state.trusted_pool_ids || []).length}/${state.trusted_pool_limit || 10}`;
  const observationInfo = `${(state.observation_pool_ids || []).length}/${state.observation_pool_limit || 10}`;
  const closedInfo = state.fail_closed ? ` | <span class="badge unavailable">严格关闭</span>` : "";
  const slotStateMap = state.slots || {};
  const slotLine = ["A", "B"].map(s => {
    const info = slotStateMap[s] || {};
    const role = state.active_slot === s ? "活动" : "待机";
    const fresh = info.verified_at ? `${Math.max(0, Math.round(Date.now() / 1000 - info.verified_at))}s前` : "无数据";
    return `${s}(${esc(info.device || "-")}|${role}): ${esc(info.node_id || "-")} 出口:${esc(info.exit_ip || "-")} 验证:${fresh}`;
  }).join("；");
  const slotFlags = `${state.rotation_paused ? " | <span class='badge unavailable'>轮换已暂停</span>" : ""}${state.fail_closed ? " | <span class='badge unavailable'>fail-closed</span>" : ""}`;
  if ($("status")) { $("status").innerHTML=`<span class="status-dot"></span>HTTP 代理本地接口：${localProxy} | 活动节点：${activeNodeInfo} | 可信池：${trustedInfo} | 观察池：${observationInfo}${closedInfo} | 双槽：${slotLine}${slotFlags} | 状态：${statusMessage}`; }
...
def check_proxy_health() -> dict[str, Any]:
    # 0. fail-closed：无活动槽时直接报错返回，不触碰本地代理端口、不发 SOCKS 请求
    active_slot = current_active_slot()
    if active_slot is None:
        return {
            "ok": False,
            "error": "[错误代码 3004] [ERR_ROUTE_DEV_NOT_FOUND] 无活动槽位（fail-closed），代理出口已阻断，等待后台恢复隧道"
        }
    active_device = slot_state.SLOT_DEVICES[active_slot]
    ...
    # 2. 检测活动槽虚拟网卡是否存在 (Linux 下)
    if sys.platform.startswith("linux") and not Path(f"/sys/class/net/{active_device}").exists():
        return {
            "ok": False,
            "error": f"[错误代码 3004] [ERR_ROUTE_DEV_NOT_FOUND] VPN 虚拟网卡 ({active_device}) 未启用，请确保当前已成功连接 VPN 节点"
        }
    ...
    while True:
        ...
        try:
            ...
            try:
                ...
                log_to_json("WARNING", "Standby", f"standby 槽维护异常: {exc}")
            ...
            if res["ok"]:
                ...
                try:
                    run_frontend_watchdog()
                except Exception as exc:
                    log_to_json("WARNING", "Frontend", f"前端观测异常: {exc}")
            else:
                ...
                # If we intended to have an active VPN node but proxy failed, run failure attribution
                if active_openvpn_node_id:
                    ui_cfg = load_ui_config()
                    routing_mode = ui_cfg.get("routing_mode", "auto")
                    if routing_mode != "fixed_ip":
...
class Handler(BaseHTTPRequestHandler):
    def get_secret_path(self) -> str:
        ui_cfg = load_ui_config()
        return ui_cfg.get("secret_path", "<SECRET_PATH>")
    ...
    def do_GET(self) -> None:
        ...
        if effective_path in ("/", "/index.html"):
            ...
            active_slot = current_active_slot()
            active_device = slot_state.SLOT_DEVICES.get(active_slot) if active_slot else None
            ovpn_ok = active_openvpn_running()
            ovpn_err = ""
            ovpn_details = "未连接"
            if ovpn_ok:
                ovpn_details = f"已连接节点: {active_openvpn_node_id} (槽位 {active_slot or '?'} / 设备 {active_device or '未知'})"
                if sys.platform.startswith("linux") and active_device:
                    if not Path(f"/sys/class/net/{active_device}").exists():
                        ovpn_err = f"[警告] 虚拟网卡 ({active_device}) 未启用，可能存在策略路由配置问题。"
            else:
                if active_openvpn_node_id:
                    ovpn_err = "连接已中断或 OpenVPN 核心程序异常退出。"
                    ovpn_details = f"尝试连接节点 {active_openvpn_node_id} 失败"
                elif active_slot is None:
                    ovpn_details = "fail-closed：无活动槽位，代理出口已阻断"
            openvpn_status = {
                "name": "OpenVPN 核心连接",
            ...
            standby_ready = int(runtime_state.get("standby_ready", 0) or 0)
            standby_target = int(runtime_state.get("standby_target", STANDBY_TARGET) or STANDBY_TARGET)
            fail_closed = bool(runtime_state.get("fail_closed", False))
            standby_status = {
                "name": "日本住宅可信池 / 观察池 / 失败关闭",
                "status": "running" if standby_ready >= standby_target and not fail_closed else "stopped",
                "details": f"可信: {len(runtime_state.get('trusted_pool_ids', []) or [])}/{node_pool.TRUSTED_LIMIT}；观察: {len(runtime_state.get('observation_pool_ids', []) or [])}/{node_pool.OBSERVATION_LIMIT}；严格关闭: {'是' if fail_closed else '否'}",
                "error": str(runtime_state.get("fail_closed_reason") or "") if fail_closed else (
                    f"住宅备用节点还差 {max(0, standby_target - standby_ready)} 个，后台会继续补齐。"
                    if standby_ready < standby_target else ""
                ),
            }
            now = time.time()
            ...
            self.send_json({
                "ok": True,
                "active_slot": runtime_state.get("active_slot"),
                "slots": runtime_state.get("slots", {}),
                "draining_slot": runtime_state.get("draining_slot"),
                "switch_cooldown_until": runtime_state.get("switch_cooldown_until", 0),
                "rotation_paused": runtime_state.get("rotation_paused", False),
                "last_switch_at": runtime_state.get("last_switch_at"),
                "last_failover_reason": runtime_state.get("last_failover_reason"),
                "services": [
                    web_ui_status,
                    proxy_gateway_status,
                    openvpn_status,
    ...
    def do_POST(self) -> None:
        ...
        if effective_path == "/api/update_credentials":
            try:
                ...
                expected_suffix = ui_cfg.get("secret_path", "<SECRET_PATH>")
            ...
            try:
                ...
                if not isinstance(fav_ids, list):
                    fav_ids = []
                opt_out_ids = ui_cfg.get("favorite_opt_out_ids", [])
                if not isinstance(opt_out_ids, list):
                    opt_out_ids = []

                if node_id in fav_ids:
                    fav_ids.remove(node_id)
                    if node_id not in opt_out_ids:
                        opt_out_ids.append(node_id)
                else:
                    fav_ids.append(node_id)
                    if node_id in opt_out_ids:
                        opt_out_ids.remove(node_id)

                ui_cfg["favorite_node_ids"] = fav_ids
                ui_cfg["favorite_opt_out_ids"] = opt_out_ids
                auth_file = DATA_DIR / "ui_auth.json"
                with lock:
                    DATA_DIR.mkdir(exist_ok=True, parents=True)
                    write_json(auth_file, ui_cfg)

                self.send_json({"ok": True, "favorite_node_ids": fav_ids, "message": "收藏仅用于人工切换，不影响可信池排序"})
            except Exception as exc:
                self.send_json({"ok": False, "error": str(exc)}, HTTPStatus.INTERNAL_SERVER_ERROR)
            return
        ...
        if effective_path == "/api/check":
            ...
            try:
                payload = self.read_json_body()
                self.send_json({"ok": True, "message": connect_node(str(payload.get("id") or ""), manual=True)})
            except Exception as exc:
                self.send_json({"ok": False, "error": str(exc)}, HTTPStatus.INTERNAL_SERVER_ERROR)
...
def main() -> None:
    global startup_preferred_node_id
    ensure_dirs()
    cleanup_test_artifacts()
    old_state = read_json(STATE_FILE, {})
    startup_preferred_node_id = str(old_state.get("active_openvpn_node_id") or "")
    if not startup_preferred_node_id:
        startup_preferred_node_id = str(next((node.get("id") for node in read_nodes() if node.get("active") and node.get("id")), ""))
    if startup_preferred_node_id:
    ...
    try:
        if log_file.exists() and log_file.stat().st_size > 50 * 1024 * 1024:
            tail = log_file.read_bytes()[-5 * 1024 * 1024:]
            temp_log = log_file.with_suffix(".log.tmp")
            temp_log.write_bytes(tail)
    ...
    write_json(
        STATE_FILE,
        {
            ...
            "standby_target": STANDBY_TARGET,
            "standby_ready": 0,
            "standby_shortage": STANDBY_TARGET,
            "standby_node_ids": [],
            "fail_closed": True,
            "fail_closed_reason": "服务启动中，尚未验证合格 VPN 出口",
        },
    )
    try:
        refresh_pool_state()
    except Exception as exc:
        log_to_json("WARNING", "Pool", f"启动时恢复节点池失败: {exc}")
    ...
    try:
        _recover_slots_on_boot()
    except Exception as exc:
        print(f"[启动恢复] 槽位恢复失败: {exc}", flush=True)
        log_to_json("ERROR", "Boot", f"启动槽位恢复失败: {exc}")
