...
PROXY_DATA_DIR = (
    str(Path(os.environ["VPNGATE_DATA_DIR"]).resolve())
    if os.environ.get("VPNGATE_DATA_DIR")
    else str(_ROOT_DIR / "vpngate_data")
)
...
class ProxyError(OSError):
    def __init__(self, code: int, message: str):
        self.code = code
        super().__init__(f"[错误代码 {code}] {message}")


def resolve_active_device(data_dir: str) -> str:
    pointer = slot_state.read_active_slot(data_dir)
    if not pointer:
        raise ProxyError(3004, "[ERR_ROUTE_DEV_NOT_FOUND] 无有效活动槽指针，7928 拒绝连接（fail-closed）：活动槽未初始化或指针非法。")
    device = pointer["device"]
    if not os.path.isdir(f"/sys/class/net/{device}"):
        raise ProxyError(3004, f"[ERR_ROUTE_DEV_NOT_FOUND] 活动槽虚拟网卡 {device} 不存在！这通常是因为 OpenVPN 核心未能成功连接或已被异常终止。")
    return device
...
def dns_query_over_tun0(host: str, qtype: int, dns_server: str, timeout: float, device: str | None = None) -> str | None:
    if device is None:
        device = resolve_active_device(PROXY_DATA_DIR)
    import random
    sock = None
    try:
...
_DNS_QTYPE_CODES = {"A": 1, "AAAA": 28}

def resolve_dns_over_tun(host: str, qtype: str = "A", dns_server: str = "8.8.8.8", timeout: float = 5, device: str | None = None) -> str | None:
    if device is None:
        device = resolve_active_device(PROXY_DATA_DIR)
    try:
    ...
    try:
        ...
        pass
    qtype_code = qtype if isinstance(qtype, int) else _DNS_QTYPE_CODES[qtype.upper()]
    return dns_query_over_tun0(host, qtype_code, dns_server, timeout, device=device)

def resolve_dns_over_tun0(host: str, dns_server: str = "8.8.8.8", timeout: float = 3.0, device: str | None = None) -> str | None:
    if device is None:
        device = resolve_active_device(PROXY_DATA_DIR)
    try:
        socket.inet_aton(host)
    ...
    try:
        ...
        pass
    return dns_query_over_tun0(host, 1, dns_server, timeout, device=device) or dns_query_over_tun0(host, 28, dns_server, timeout, device=device)

def create_connection(address: tuple[str, int], timeout: float = 20, device: str | None = None) -> socket.socket:
    if device is None:
        device = resolve_active_device(PROXY_DATA_DIR)
    host, port = address
