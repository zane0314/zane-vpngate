...
发布前按部署环境填写：VPS 服务商 `<VPS_PROVIDER_URL>`、支持渠道 `<SUPPORT_CHANNEL_URL>`、问题反馈邮箱 `<CONTACT_EMAIL>`。
...
正式安装器默认使用 `OWNER/REPOSITORY` 的日本住宅可信池分支。公开前请将仓库占位符替换为实际地址；安装器启用严格住宅失败关闭、10 个可信池、10 个观察池、每日维护，并在结束时执行安装验收。

```bash
bash <(curl -fsSL https://raw.githubusercontent.com/OWNER/REPOSITORY/main/install-zane.sh)
```

安装前必须确认 VPS 已启用 `/dev/net/tun`。该安装器只部署 AimiliVPN 本地出口代理，不会自行修改 3x-ui、nginx、Cloudflare 或现有订阅；需要“CDN 入口 → VPNGate 出口”时，应在原 3x-ui 入站与订阅管线上单独接入。

如果使用本地正式发布归档，将归档上传到 VPS、解压后执行 `bash install-local-package.sh`。这种方式不依赖 GitHub 上是否已经发布最新安装器。

详细的安装、升级、功能修改和验收解释见：[安装与修改说明](docs/安装与修改说明.md)。

#### 🌟 正式稳定版本 (main 分支)
```bash
bash <(curl -Ls https://raw.githubusercontent.com/OWNER/REPOSITORY/main/install.sh)
```
> 💡 **小贴士**：部署完成后，终端会输出管理网页的专属链接（含随机安全后缀，如 `http://<VPS_IP>:8787/<RANDOM_SECRET_PATH>`）。在终端中输入 `ml` 命令可以随时调出交互式命令行管理菜单。
...
当网页中选择“住宅 IP”时，只接受候选隧道真实出口为日本 IPv4 `residential` 且无明确硬风险的节点，不会将 VPNGate 服务器地址误当作落地出口。IP 质量采用可审计的四段加权分：`ip.net.coffee` 的 ASN/ISP、住宅与 VPS 属性占 40%；CleanIP 的滥用、黑名单和攻击历史占 30%；CleanIP 的匿名、VPN 与代理特征占 20%；CleanIP `purity.score` 单站分仅占 10%。缺少一个接口时会标记为降级评分，不伪装成完整结果。可信池和观察池各限制 10 个；排序先比较 IP 质量，质量差不超过 5 分时才允许 30% 以上的下载速度优势改变选择，并要求连续两轮胜出后切换。当没有合格出口时，本地代理严格失败，不回退到 VPS 默认出口。

主动维护每天在系统本地时间 03:00—04:00 执行：活动节点和两个最优挑战节点各进行两次 25 MB 下载及 3 MB 上传测试，另对超过七天未测的池节点轮换一台；新节点与补池采用一次 10 MB 下载。测速目标为 Cloudflare speed endpoint，候选阶段记录 `VPNGate IPv4 → Cloudflare`，同时通过现有两条订阅测试 `XHTTP/CDN → 3x-ui/nginx → VPS 直出` 与 `XHTTP/CDN → 3x-ui/nginx → AimiliVPN → VPNGate` 两条完整路径。测试实际传输量按日、按月记入状态，仅作观测，不设置硬预算。

| 环境变量 | 默认值 | 作用 |
| --- | ---: | --- |
| `STRICT_RESIDENTIAL_ONLY` | `true` | 住宅筛选只允许 `residential` |
| `TRUSTED_POOL_LIMIT` | `10` | 可信池硬上限 |
| `OBSERVATION_POOL_LIMIT` | `10` | 观察池硬上限 |
| `STANDBY_TARGET` | `10` | 自动切换所需的可信/观察备用目标数 |
| `STANDBY_MAX_AGE_SECONDS` | `691200` | 备用节点测试结果最长有效时间（8 天） |
| `STANDBY_SCAN_BATCH_SIZE` | `10` | 每批预热测试的候选节点数 |
| `STANDBY_SCAN_LIMIT` | `20` | 单轮最多测试的节点数 |
| `TEST_MAX_WORKERS` | `2` | 隔离隧道测试最大并发数 |
| `DAILY_MAINTENANCE_START_HOUR` | `3` | 每日主动维护开始小时 |
| `DAILY_MAINTENANCE_END_HOUR` | `4` | 每日主动维护结束小时 |
| `STABLE_POOL_PROBE_DAILY_LIMIT` | `2` | 稳定池每日轮换短测节点上限 |
| `STABLE_POOL_PROBE_BYTES` | `25000000` | 活动节点和最优挑战节点每个样本的下载量（25 MB） |
| `STABLE_POOL_PROBE_UPLOAD_BYTES` | `3000000` | 每个性能样本的上传量（3 MB） |
| `STABLE_POOL_PROBE_SAMPLES` | `2` | 性能测试样本数，使用中位数 |
| `FULL_PROBE_BYTES` | `10000000` | 新节点、补池与手动测试下载量（10 MB） |
| `QUALITY_TIE_WINDOW` | `5` | 加权 IP 质量差不超过此值才比较速度 |
| `SPEED_SWITCH_GAIN_PERCENT` | `30` | 质量接近时触发候选胜出的最低速度优势 |
| `SWITCH_REQUIRED_WINS` | `2` | 自动择优切换前所需连续胜出轮数 |
...
Configure `<VPS_PROVIDER_URL>`, `<SUPPORT_CHANNEL_URL>`, `<SUPPORT_FORUM_URL>`, and `<CONTACT_EMAIL>` before publishing deployment-specific documentation.
...
Open your browser and navigate to the printed URL (e.g. `http://<VPS_IP>:8787/<RANDOM_SECRET_PATH>`).
